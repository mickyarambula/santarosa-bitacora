...
async function report(sql: Sql, me: Profile, day: string) {
  ...
  const [events, producers, tasks, people] = await Promise.all([
    sql<Record<string, unknown>>`with events as (
      select t.id,t.producer_id,t.owner_user_id as actor_id,'contacto' as kind,
        concat(t.channel,' · ',t.outcome,' · ',t.summary) as detail,t.happened_at as at
        from touches t where t.channel<>'nota'
      union all
      select c.id,c.producer_id,coalesce(c.confirmed_by,c.created_by),'contacto',
        concat(c.channel,' · ',case when c.status='recibido' then 'Recibido' else 'Envío confirmado manualmente' end,' · ',c.body),c.happened_at
        from producer_communications c where c.status in ('recibido','enviado_manual')
      union all
      select a.id,a.producer_id,a.user_id,a.kind,a.message,a.created_at from activity a
        where a.kind in ('alta','etapa')
      union all
      select a.id,d.producer_id,a.actor_user_id,'documento',concat(d.doc_type,' · ',a.after_data->>'status'),a.created_at
      ...
      union all
      select a.id,v.producer_id,a.actor_user_id,'visita',concat(v.purpose,' · ',a.after_data->>'notes'),a.created_at
    ...
    ) select e.*,p.name,p.owner_user_id,p.portfolio_kind,p.comisionista_name,
    ...
    sql<{ user_id: string; display_name: string }>`select p.user_id,p.display_name from profiles p
      where p.role in ('comisionista','gerente') and p.status='activo' and p.merged_into_user_id is null
  ...
  const mappedEvents: WeekEvent[] = events.map((e) => ({
    id: String(e.id),
