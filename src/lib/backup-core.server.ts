...
export function backupTablesForMigrations(migrations: readonly string[]) {
  return migrations.includes("0013_traceability.sql")
    ? [...BACKUP_TABLES]
    : BACKUP_TABLES.filter((t) => t !== "crm_audit");
}
...
export function assertBackupRuntime(
  ...
  if (!databaseConfigured)
    throw new BackupError(
      "Esta vista previa no contiene la base publicada. No se generó un respaldo.",
    );
...
export async function collectBackup(
  ...
  try {
    ...
    if (
      ...
      access.rows[0]?.role !== "gerente" ||
      access.rows[0]?.status !== "activo"
    ) {
    ...
    if (
      !sameNames(
        ...
        backupTablesForMigrations(expectedMigrations),
      ) ||
      tables.rows.some((r) => r.relkind !== "r")
    ) {
      throw new BackupError(
        "Hay tablas o estructuras distintas al código revisado. No se generó un respaldo parcial.",
      );
    }
    ...
    let bytes = 0;
    for (const name of backupTablesForMigrations(expectedMigrations)) {
      ...
      const result = await query(
        `select row_to_json(t)::text as row from public.${quoteIdentifier(name)} t limit ${MAX_TABLE_ROWS + 1}`,
      );
      if (result.rows.length > MAX_TABLE_ROWS)
        throw new BackupError("El volumen requiere otro método de respaldo.");
...
export function encryptBackup(snapshot: BackupSnapshot, recipientPublicKey: string): string {
  const header = {
    format: "santarosa-encrypted",
    version: 1,
    algorithm: "RSA-OAEP-256+AES-256-GCM",
    recipient: fingerprint(recipientPublicKey),
  };
  ...
  try {
    ...
    const encryptedKey = publicEncrypt(
      { key: recipientPublicKey, oaepHash: "sha256", padding: constants.RSA_PKCS1_OAEP_PADDING },
      key,
    );
    ...
    if (Buffer.byteLength(envelope) > MAX_RESPONSE_BYTES)
      throw new BackupError("El volumen requiere otro método de respaldo.");
    return envelope;
  } finally {
    key.fill(0);
...
export function decryptBackup(envelope: string, privateKey: string): BackupSnapshot {
  if (Buffer.byteLength(envelope) > MAX_RESPONSE_BYTES)
    throw new BackupError("Archivo demasiado grande.");
  ...
  if (
    ...
    algorithm !== "RSA-OAEP-256+AES-256-GCM" ||
    recipient !==
      fingerprint(createPublicKey(privateKey).export({ type: "spki", format: "pem" }).toString())
  ) {
  ...
  try {
    ...
    const snapshot = JSON.parse(
      gunzipSync(compressed, { maxOutputLength: MAX_PLAIN_BYTES }).toString("utf8"),
    );
...
export function validateBackup(snapshot: BackupSnapshot) {
  if (
    !snapshot ||
    snapshot.format !== "santarosa-data" ||
    snapshot.version !== 1 ||
    !Array.isArray(snapshot.tables) ||
    !Array.isArray(snapshot.migrations) ||
    !sameNames(
      snapshot.tables.map((t) => t.name),
      backupTablesForMigrations(snapshot.migrations),
    )
  ) {
  ...
  for (const table of snapshot.tables) {
    if (
      ...
      table.rows.some((r) => typeof r !== "string")
    ) {
      throw new BackupError("Respaldo incompleto o formato incompatible.");
    }
  }
