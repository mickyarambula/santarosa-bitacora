...
import pg from 'pg';
import { BACKUP_TABLES, decryptBackup, quoteIdentifier, readBackupColumns } from '../src/lib/backup-core.server.ts';
...
export async function restoreIntoEmptyDatabase(client, snapshot, migrationFiles) {
  ...
  try {
    await client.query("set local timezone = 'UTC'; set local datestyle = 'ISO, YMD'; set local statement_timeout = '30s'");
    ...
    for (const table of snapshot.tables) {
      ...
      if (JSON.stringify(columns) !== JSON.stringify(table.columns)) throw new Error('Estructura incompatible.');
    }
    ...
    for (const name of BACKUP_TABLES) {
      ...
      for (const row of table.rows) {
        await client.query(`insert into public.${quoteIdentifier(name)} select * from json_populate_record(null::public.${quoteIdentifier(name)}, $1::json)`, [row]);
      }
    }
    const counts = {};
    for (const table of snapshot.tables) {
      const rows = (await client.query(`select row_to_json(t)::text as row from public.${quoteIdentifier(table.name)} t`)).rows.map(r => r.row).sort();
      if (JSON.stringify(rows) !== JSON.stringify([...table.rows].sort())) throw new Error('La recuperación no coincide.');
...
if (process.argv[1]?.endsWith('/restore-migration-backup.mjs')) {
  ...
  let client;
  try {
    if (!receiptPath || !expectedHost?.endsWith('.neon.tech') || expectedHost.includes('-pooler')) throw new Error('Faltan argumentos de destino directo.');
    const env = parseEnv(await readFile(envPath, 'utf8'));
    const url = new URL(env.DATABASE_URL_UNPOOLED);
    if (url.hostname !== expectedHost || env.NEON_PROJECT_ID !== 'soft-sun-73911653') throw new Error('Destino diferente de Santa Rosa.');
    ...
    const files = await Promise.all(names.map(async name => ({name, sql: await readFile(new URL(name, dir), 'utf8')})));
    ...
    const receipt = { restoredAt: new Date().toISOString(), destinationHost: expectedHost,
      sourceCreatedAt: snapshot.source.createdAt, sha256: createHash('sha256').update(bytes).digest('hex'), ...result };
    await writeFile(receiptPath, JSON.stringify(receipt, null, 2)+'\n', {mode: 0o600, flag:'wx'});
    console.log(JSON.stringify(receipt, null, 2));
  } catch (error) {
    // Driver errors may contain personal data or credentials. Only expose code.
    console.error('Restauración detenida; revisa destino vacío, versión y permisos.', error?.code ?? 'VALIDATION');
    process.exitCode = 1;
