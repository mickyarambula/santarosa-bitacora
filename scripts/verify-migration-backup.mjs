...
export async function createIsolatedBackupDatabase(selectedMigrations) {
  const db = new PGlite();
  try {
    await db.waitReady;
    ...
    const available = (await readdir(migrationsDir))
      ...
      .sort();
    const migrations = selectedMigrations ? [...selectedMigrations].sort() : available;
    if (JSON.stringify(available.slice(0, migrations.length)) !== JSON.stringify(migrations))
...
export async function verifyBackupInMemory(snapshot) {
  ...
  const { db, migrations } = await createIsolatedBackupDatabase(snapshot.migrations);
  try {
    ...
    const history = (await db.query("select name from _migrations order by name")).rows.map(
      (r) => r.name,
    );
    if (JSON.stringify(history) !== JSON.stringify(migrations))
      throw new Error("Historial de migraciones inconsistente.");
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (!backupPath || !privateKeyPath) {
    console.error(
      "Uso: node --experimental-strip-types scripts/verify-migration-backup.mjs archivo.srbackup llave.pem",
    );
    process.exitCode = 1;
  } else {
    try {
      ...
      console.log(
        "Respaldo descifrado y restaurado íntegramente en memoria. Ninguna base remota fue consultada.",
      );
      ...
      console.error(
        "No se pudo validar el respaldo. Revisa archivo, llave y versión del código; no se modificó ninguna base remota.",
      );
      process.exitCode = 1;
    }
  }
